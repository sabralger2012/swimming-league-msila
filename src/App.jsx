...
function SwimmerCardFront({ swimmer }) {
  return (
    <div
      ...
      style={{ '--card-bg': `url("${CARD_FEDERATION_LOGO}")` }}
    >
      ...
      <div className="card-body">
        ...
        <div className="card-photo">
          <img src={swimmer.photo_url || '/default-avatar.png'} alt="السباح" className="card-swimmer-photo" />
        </div>
      </div>
    </div>
  )
...
function App() {
  ...
  async function saveUser(e) {
    ...
    if (userForm.role === 'club_admin' && !userForm.club_id) {
      return setError('يجب اختيار النادي لرئيس النادي.')
    }
    ...
    if (userModal === 'add') {
      ...
      // نفس أسلوب إنشاء رئيس النادي عند إضافة نادٍ جديد:
      // supabase.auth.signUp() يستبدل جلسة الدخول الحالية، لذا نحفظ جلسة الرئيس الحالي
      ...
      try {
        ...
        else { newUserId = signUpData?.user?.id || null }
      ...
      // بعض إعدادات قاعدة البيانات قد تُنشئ صف profiles بدور افتراضي (رئيس نادٍ) بغض النظر
      // عن الدور المُرسل عند التسجيل، لذا نُصحّح الدور والنادي هنا بعد استعادة جلسة الرئيس،
      // للتأكد أن الدور الذي اخترته فعليًا فعلاً هو الذي يُحفظ.
      if (newUserId) {
        const { error: fixErr } = await supabase.from('profiles').update({
          full_name: userForm.full_name.trim(),
          role: userForm.role,
          club_id: userForm.role === 'club_admin' ? userForm.club_id : null,
        }).eq('id', newUserId)
        if (fixErr) setError(fixErr.message)
      }
  ...
  async function deleteUser(user) {
    ...
    if (user.id === profile?.id) {
      return setError('لا يمكنك حذف حسابك الخاص أثناء تسجيل الدخول به.')
    }
    if (!confirm(`هل أنت متأكد من حذف حساب "${user.full_name || user.email}"؟ سيفقد هذا المستخدم صلاحية الوصول إلى لوحة التحكم فورًا.`)) return
  ...
  async function saveClub(e) {
    if (!isFederationAdmin && clubModal === 'add') return setError('رئيس النادي لا يمكنه إضافة نادٍ.')
    e.preventDefault()
    setError('')
    const payload = {
    ...
    if (clubModal === 'add' && clubForm.admin_email && clubForm.admin_password) {
      ...
      // نحفظ جلسة رئيس الاتحادية أولاً حتى نعيدها بعد إنشاء حساب رئيس النادي،
      // فلا يتم تسجيل خروج الرئيس الحالي أو إدخاله بحساب النادي الجديد.
      const { data: currentSessionData } = await supabase.auth.getSession()
      const adminSession = currentSessionData?.session
      ...
      try {
        ...
        if (authErr) setError(`تم حفظ النادي ولكن تعذر إنشاء حساب الرئيس: ${authErr.message}`)
        else newAdminId = signUpData?.user?.id || null
      ...
      // تأكيد إضافي: نضمن أن دور الحساب الجديد ونادِيه محفوظان بشكل صحيح
      if (newAdminId) {
...
function UsersPage({ users, search, setSearch, onAdd, onEdit, onDelete, error }) {
  const roleLabel = (role) => role === 'federation_admin' ? 'رئيس الاتحادية' : 'رئيس نادي'
      ...
      <table>
        ...
        <tbody>
          {users.map(u => <tr key={u.id}>
            ...
            <td>
              <button className="small" onClick={() => onEdit(u)}>تعديل الصلاحيات</button>
              <button className="small danger" onClick={() => onDelete(u)}>حذف</button>
            </td>
          </tr>)}
    ...
    <form onSubmit={onSave} className="form-grid">
      ...
      <label>الدور *
        <select value={form.role} onChange={e => update('role', e.target.value)}>
          <option value="federation_admin">رئيس الاتحادية</option>
          <option value="club_admin">رئيس نادي</option>
        </select>
      </label>
...
function ClubModal({ form, setForm, onClose, onSave, editing, error }) {
  return <Modal title={editing ? 'تعديل النادي' : 'إضافة نادي ومعلومات الرئيس'} onClose={onClose}>
    {error && <div className="alert">{error}</div>}
    <form onSubmit={onSave} className="form-grid two">
      ...
      {!editing && (
        <>
          <h4 className="wide" style={{marginTop: '1rem', borderTop: '1px solid #ddd', paddingTop: '0.5rem'}}>معلومات رئيس النادي (حساب الدخول)</h4>
          ...
          <label>البريد الإلكتروني للرئيس *<input type="email" value={form.admin_email || ''} onChange={e => setForm({...form, admin_email: e.target.value})} /></label>
          <label>كلمة المرور للرئيس *<input type="password" value={form.admin_password || ''} onChange={e => setForm({...form, admin_password: e.target.value})} /></label>
        </>
      )}
